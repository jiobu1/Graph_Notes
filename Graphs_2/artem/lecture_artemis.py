...
class Graph:
    ...
    def bfs(self, starting_vertex, destination_vertex):
        ...
        # Create an empty set to track visited vertices
        queue.enqueue({
            "current_vertex" : starting_vertex, # queue must store vertices
            "path" : [starting_vertex]
        })
        # queue.enqueue([starting_vertex]) - alternative
        # while the queue is not empty:
        while queue.size() > 0: # check len, not empty
            # get the current vertex path(deque from queue)
            current_object = queue.dequeue()
            current_path = current_object['path'] # <-- not in breadth-first traversal
            current_vertex = current_object['current_vertex'] # *
            ...
            if current_vertex not in visited_vertices: # *
                ...
                for neighbor_vertex in self.get_neighbors(current_vertex): # find all vertices that are connected and pop them onto the queue and queue them up
    ...
    def dfs(self, starting_vertex, destination_vertex):
        ...
        # while the stack is not empty:
        while stack.size() > 0: # check len, not empty
